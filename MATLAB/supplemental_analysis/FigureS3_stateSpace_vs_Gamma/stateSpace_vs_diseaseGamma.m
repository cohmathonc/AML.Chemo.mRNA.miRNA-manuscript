...
% ===== Untreated potential drift term =====
SDE = @(t, x, gamma) [-((2.*x(1).*(((x(1)-c1).^2 + (x(2)-c1).^2) + gamma)) + ((x(1).^2 + x(2).^2) .* (2.*(x(1) - c1)))); -((2.*x(2).*(((x(1)-c1).^2 + (x(2)-c1).^2)+ gamma)) + (2.*(x(2)-c1).*(x(1).^2 + x(2).^2)))];

% ===== Treated potential drift term =====
SDE_chemo = @(t,x,effect,gamma) [-alpha.*(((2*psi(1,1).*x(1) + (psi(1,2) + psi(2,1)).*x(2)).*((phi(1,1).*(x(1)-c1).^2 + (phi(1,2) + phi(2,1)).*(x(1)-c1).*(x(2)-c1) + phi(2,2).*(x(2)-c1).^2) + gamma)) + (psi(1,1).*x(1).^2 + (psi(1,2) + psi(2,1)).*x(1).*x(2) + psi(2,2).*x(2).^2 + rr.*effect).*(2*phi(1,1).*(x(1)-c1) + (phi(1,2) + phi(2,1)).*(x(2)-c1))); -alpha.*((((psi(1,2) + psi(2,1)).*x(1) + 2*psi(2,2).*x(2)).*((phi(1,1).*(x(1)-c1).^2 + (phi(1,2) + phi(2,1)).*(x(1)-c1).*(x(2)-c1) + phi(2,2).*(x(2)-c1).^2) + gamma)) + ((psi(1,1).*x(1).^2 + (psi(1,2) + psi(2,1)).*x(1).*x(2) + psi(2,2).*x(2).^2 + rr.*effect).*((phi(1,2) + phi(2,1)).*(x(1)-c1) + 2*phi(2,2).*(x(2)-c1))))];
...
% Store if cured or not
counter_cured_mRNA = zeros(length(x0_statespace),length(gamma_values));
counter_cured_miRNA = zeros(length(x0_statespace),length(gamma_values));
counter_cured_both = zeros(length(x0_statespace),length(gamma_values));
...
for j = 1:length(dose_factors)
    ...
    for g = 1:length(gamma_values)
        ...
        for k = 1:length(x0_statespace)
            ...
            for i = 2:length(t)
                ...
                if t(i) <= w0
                  ...
                  F = (X.^2 + Y.^2).*(((X-c1).^2 + (Y-c1).^2) + gamma);
                    ...
                    Drift = SDE_chemo(t(i-1), x(:, i-1),effect,gamma); %apply chemo
            ...
            % Record cured or not based on gamma value
            if x(1,end)>c2_mRNA
                counter_cured_mRNA(k,g) = 1;
            else
                counter_cured_mRNA(k,g) = 0;
            end

            if x(2,end)>c2_miRNA
                counter_cured_miRNA(k,g) = 1;
            else
                counter_cured_miRNA(k,g) = 0; 
            end

            if (x(1,end)>c2_mRNA && x(2,end)>c2_miRNA)
                counter_cured_both(k,g) = 1;
            else
                counter_cured_both(k,g) = 0; 
            end
        end
    end
end
